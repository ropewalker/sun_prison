...
type MovedEnemy = (With<Enemy>, Changed<GameCoordinates>);

pub fn enemy_sprite_update_system(
    mut query: Query<(&mut GameCoordinates, &mut TextureAtlasSprite), MovedEnemy>,
) {
    const UP: u32 = 3;
    const DOWN: u32 = 0;
    const RIGHT: u32 = 2;
    ...
    for (coordinates, mut sprite) in query.iter_mut() {
